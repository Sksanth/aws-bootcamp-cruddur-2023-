...
## Required work
In the ```backend-flask/requirement.txt ``` add ```boto3``` and run the command ``` pip install -r requirements.txt```
### DynamoDB Utility Scripts
#### Create a folder called ```ddb``` under ```backend-flask/bin``` and create the following scripts
**"schema-load"**
```
#!/usr/bin/env python3
...
def get_user_uuids():
  ...
  my_user    = next((item for item in users if item["handle"] == 'sasi'), None)
  other_user = next((item for item in users if item["handle"] == 'thv'), None)
  results = {
    'my_user': my_user,
...
./bin/ddb/scan
``` 
#### Create a new folder ```patterns``` inside ```bin/ddb``` and create 2 files
**"get-conversation"**
```
#!/usr/bin/env python3
...
Execute
 ...
 ```
 ![list-convo](https://user-images.githubusercontent.com/102387885/227828592-880326d7-20ed-421a-b304-52150c0ecfc9.png)
...
```
Execute
``` chmod u+x bin/ddb/patterns/list-conversation
./bin/ddb/patterns/list-conversation
```
#### Create a folder ```cognito``` under ```backend-flask/bin``` and create a file
**"list-users"**
```
#!/usr/bin/env python3
